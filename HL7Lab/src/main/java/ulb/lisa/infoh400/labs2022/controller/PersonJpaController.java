...
public class PersonJpaController implements Serializable {
    ...
    public Person findDuplicate (Person person){
        EntityManager em = getEntityManager();
        List rst = em.createNamedQuery("Person.findDuplicate").setParameter("firstname",person.getFirstname()).setParameter("familyname",person.getFamilyname()).setParameter("dateofbirth", person.getDateofbirth()).getResultList();
        if (!rst.isEmpty()){
            return (Person) rst.get(0);
        }
        return null;
    }
